...
pub struct Interpolator<const N: usize> {
    /// Sorted input values
    inputs: Vec<f64>,
    /// Corresponding output values (sorted by input)
    outputs: Vec<[f64; N]>,
}
...
impl<const N: usize> Interpolator<N> {
    ...
    ///
    /// ## Arguments
    /// * `inputs` - Input values (will be sorted)
    /// * `outputs` - Corresponding output values (will be sorted to match inputs)
    ///
    /// ## Returns
    /// * `Ok(Interpolator)` if data is valid
    /// * `Err(String)` if validation fails
    ///
    /// ## Validation
    /// * Input and output arrays must have the same length
    /// * At least one data point is required
    /// * Input values must be unique (no duplicates after sorting)
    ///
    /// ## Example
    /// ```
    /// use spdcalc::math::Interpolator;
    ...
    /// ```
    pub fn new(inputs: Vec<f64>, outputs: Vec<[f64; N]>) -> Result<Self, String> {
        ...
        match self.inputs.binary_search_by(|probe| {
            ...
            Err(insert_idx) => {
                ...
                // Interpolate between insert_idx-1 and insert_idx
                    ...
                    for i in 0..N {
                        result[i] = lerp(self.outputs[i0][i], self.outputs[i1][i], t);
                    }
...
mod tests {
    ...
    fn test_new_empty_data() {
        ...
        assert!(result.unwrap_err().contains("At least one data point"));
    }
    ...
    fn test_interpolate_single_point() {
        ...
        assert_eq!(interp.interpolate(0.0), [42.0]);
        assert_eq!(interp.interpolate(5.0), [42.0]);
        assert_eq!(interp.interpolate(10.0), [42.0]);
    }
