...
fn create_interpolated_crystal() -> CrystalType {
  ...
  let wavelengths = vec![1500.0, 1520.0, 1540.0, 1560.0, 1580.0, 1600.0]
    .into_iter()
    .map(|nm| nm * NANO * M)
    .collect::<Vec<_>>();
  let indices = wavelengths
    .iter()
    .map(|lambda| CrystalType::BBO_1.get_indices(*lambda, from_celsius_to_kelvin(20.0)))
    .collect::<Vec<_>>();
